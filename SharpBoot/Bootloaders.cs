        ...
        public override void Install(string l)
        {
            ...
            using (
                var device = Utils.CreateFile(Utils.GetPhysicalPath(l.ToLower().Substring(0, 2)),
                    ...
                    FileShare.Write | FileShare.Read | FileShare.Delete, IntPtr.Zero, FileMode.Open,
                    Utils.FILE_ATTRIBUTE_SYSTEM | Utils.FILE_FLAG_SEQUENTIAL_SCAN, IntPtr.Zero))
            {
                if (device.IsInvalid)
                {
